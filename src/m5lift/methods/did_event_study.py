...
def event_study(df: pl.DataFrame, min_lead: int = -14, max_lag: int = 28, omit: int = -1) -> pl.DataFrame:
    ...
    mask = (rel >= min_lead) & (rel <= max_lag) & (rel != omit)
    unit = unit[mask]
    time = time[mask]
    y = y[mask]
...
def main():
    ...
    pre = es.filter((pl.col("rel_day") <= -2) & (pl.col("rel_day") >= -14)).select(pl.col("beta").mean()).item()
    pre = float(pre) if pre is not None else float("nan")
