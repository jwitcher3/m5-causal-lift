...
def main():
    ...
    y_pre = pre["y_treated"].to_numpy().astype(np.float64)
    X_pre = pre.select(donor_cols).to_numpy().astype(np.float64)

    w = _ridge_weights(X_pre, y_pre, alpha=args.alpha)

    X_all = panel.select(donor_cols).to_numpy().astype(np.float64)
    y0_hat = X_all @ w

    y_all = panel["y_treated"].to_numpy().astype(np.float64)
    in_c = panel["in_campaign"].to_numpy().astype(np.int64)

    att_hat = float(np.mean((y_all - y0_hat)[in_c == 1]))
